...
int main() {
    ...
    for(const auto& name : queueNames) {
        device.getOutputQueue(name, 2, false);
    }
    ...
    while(true) {
        ...
        for(const auto& name : queueNames) {
            if(latestPacket.find(name) != latestPacket.end()) { // If found
                ...
                std::cout << "Size of frame: " << sizeof(frame[name]) << " name: " << name << " bytes" << std::endl;
            }
        }
